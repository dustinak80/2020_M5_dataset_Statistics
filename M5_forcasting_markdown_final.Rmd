...
paired.wilcox = function(df, p_adjust){
  ...
  #note: Use p_adjust = 1 for adjusting the p-value with bonferroni
  ...
  colnames(mat) = factors[1:6]
  rownames(mat) = factors[2:7]
  pvalue = c()
  tracker = c()
  count = 1
  for (i in factors){
  ...
  if (p_adjust){
    mat = matrix(p.adjust(wday.wilcox), nrow = 6, ncol = 6, byrow = FALSE)
    colnames(mat) = factors[1:6]
    rownames(mat) = factors[2:7]
    return(mat)
  }
...
#### Start looking at difference between weekday and weekend then move up above weekday comparison.
