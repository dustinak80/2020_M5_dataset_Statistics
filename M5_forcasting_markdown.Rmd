...
```
A QQ plot shows whether or not CA sales are normally distributed.
```{r}
...
A QQ plot shows whether or not Texas sales are normally distributed.
```{r}
...
```
A QQ plot shows whether or not Wisconsin Sales are normally distributed.
```{r}
...
```
We see that the average sales data by state is not normally distributed.
```{r}
mvn(weekly_sales, multivariatePlot = 'qq')
```
We use a non-parametric test for testing the group means. Conclude that there at least one State's average sales is significantly different
```{r}
